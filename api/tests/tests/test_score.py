...
class TestSubmitExamScore:
    def test_submit_exam_score_by_candidate_fail(
        ...
        score_data = {"candidate_id": candidate.pk, "score": 5}
        api_client.credentials(HTTP_AUTHORIZATION=f"Bearer {access}")
        response = api_client.put(
            submit_exam_score_url(exam.id), score_data, format="json"
        )
    ...
    def test_submit_exam_score_by_moderator_fail(
        self,
        api_client,
        submit_exam_score_url,
        create_logged_in_moderator,
        create_logged_in_screening_candidate,
    ):
