...
def api_root(request, format=None):
    ...
    def generate_url_with_placeholder(name, placeholder, param):
        ...
        try:
            dummy_id = 99999
            url = reverse(name, kwargs={param: dummy_id}, request=request, format=format)
    ...
    return Response({
        ...
        "candidates": {
            ...
            "detail": generate_url_with_placeholder('v1:api-candidate-detail', "<candidate_id>", "candidate_id"),
            "actions": {
                "assign_role": generate_url_with_placeholder('v1:api-candidate-role-assign', "<candidate_id>", "candidate_id"),
                "scores": generate_url_with_placeholder('v1:api-candidate-scores', "<candidate_id>", "candidate_id"),
                "exam_history": generate_url_with_placeholder('v1:api-candidate-exam-history', "<candidate_id>", "candidate_id"),
            }
        },
        "staff": {
            "collection": safe_reverse('v1:api-staff-list'),
            "me": safe_reverse('v1:api-staff-me'),
            "detail": generate_url_with_placeholder('v1:api-staff-detail', "<staff_id>", "staff_id"),
            "actions": {
                "assign_role": generate_url_with_placeholder('v1:api-staff-role-assign', "<staff_id>", "staff_id"),
            }
        },
        "exams": {
            "collection": safe_reverse('v1:api-exam-list'),
            "detail": generate_url_with_placeholder('v1:api-exam-detail', "<exam_id>", "exam_id"),
            "questions": generate_url_with_placeholder('v1:api-exam-questions', "<exam_id>", "exam_id"),
            "scores": {
                "submit": generate_url_with_placeholder('v1:api-exam-score-submit', "<exam_id>", "exam_id"),
            }
        },
        "questions": {
            "collection": safe_reverse('v1:api-question-list'),
            "detail": generate_url_with_placeholder('v1:api-question-detail', "<question_id>", "question_id"),
        },
        "dashboard": {
        ...
        "user_accounts": {
            ...
            "account-management-detail": generate_url_with_placeholder('v1:api-account-management-detail', "<user_id>", "user_id"),
        },
...
def login_api(request):
    ...
    if user is not None:
        ...
        logger.info(f"User {username} logged in successfully as {user_type}")
...
def logout_api(request):
    ...
    try:
        ...
        logger.warning(f"Token error during logout for user {request.user.username}: {str(e)}")
        return Response({
            'error': 'Invalid or expired token'
        }, status=status.HTTP_400_BAD_REQUEST)
        
    except InvalidToken as e:
        logger.warning(f"Invalid token during logout for user {request.user.username}: {str(e)}")
        return Response({
            'error': 'Invalid token format'
        }, status=status.HTTP_400_BAD_REQUEST)
        
    except Exception as e:
        logger.error(f"Unexpected error during logout for user {request.user.username}: {str(e)}", exc_info=True)
        return Response({
